    ...
    public Set<Square> getLShapedMovableSquares(){
        ...
        for (int xDirection = -1; xDirection <= 1; xDirection += 2) {
            for (int yRelativeDirection = -1; yRelativeDirection <= 1; yRelativeDirection += 2) {
                int x1 = piece.getSquare().getHorizontalPosition() + xDirection;
                int x2 = piece.getSquare().getHorizontalPosition() + 2 * xDirection;
                int y1 = piece.getSquare().getVerticalPosition() + 2 * xDirection * yRelativeDirection;
                int y2 = piece.getSquare().getVerticalPosition() + xDirection * yRelativeDirection;

                if (0 <= x1 && x1 < board.getHorizontalSize() && 0 <= y1 && y1 < board.getVerticalSize()){
                    Square currentSquare = board.getSquareByPos(x1, y1);
                    if (currentSquare.getPiece() == null ||
                            currentSquare.getPiece().getTeam() != piece.getTeam()) {
                        movableSquares.add(currentSquare);
                    }
                }
                if (0 <= x2 && x2 < board.getHorizontalSize() && 0 <= y2 && y2 < board.getVerticalSize()){
                    Square currentSquare = board.getSquareByPos(x2, y2);
                    if (currentSquare.getPiece() == null ||
                            currentSquare.getPiece().getTeam() != piece.getTeam()) {
                        movableSquares.add(currentSquare);
                    }
